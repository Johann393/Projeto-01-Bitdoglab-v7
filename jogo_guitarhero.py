...
# ===== MÚSICAS =====
melody_menu = [
("DO",200),("MI",200),("SOL",400),
("MI",200),("DO",400)
]

melody_normal = [
("MI",150),("MI",150),("MI",150),
("DO",150),("MI",150),("SOL",300),("SOL",300)
]
...
def draw(nx,ny,nc,px):
    clear()
    colors=[(50,0,0),(0,0,50),(0,50,0)]
    
    if 0 <= nx < 5 and 0 <= ny < 5:
        np[index(nx,ny)] = colors[nc]
    ...
    if state == MENU:
        ...
        if btn != -1:
            if menu_option == 0:
                ...
                score = 0
                player_x = 2
                note_x = random.randint(0,4)
                note_y = 0
                note_color = random.randint(0,2)
                last_move = time.ticks_ms()
                
                # 🎵 muda música conforme dificuldade
                if difficulty == 0:
                    current_melody = melody_normal
                else:
                    current_melody = melody_hard
        ...
        if btn != -1:
            if note_x == player_x and btn == note_color:
                if note_y >= 3:
                    score += 1
                    ripple(player_x,4)
                    note_x = random.randint(0,4)
        ...
        if note_y > 4:
            ...
            draw(note_x,note_y,note_color,player_x)
